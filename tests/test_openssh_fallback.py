...
def test_iexpress_bundles_fallback_msi_from_manifest():
    assert MANIFEST["openssh"]["filename"] == MSI
    assert MANIFEST["openssh"]["sha256"] == SHA
    assert "payload_manifest.json" in BUILD
    assert "FILE2=" in BUILD and "%FILE2%=" in BUILD


def test_build_fetches_only_pinned_official_msi_and_verifies_it():
    assert "PowerShell/Win32-OpenSSH/releases/download/v9.8.3.0p2-Preview/" in MANIFEST["openssh"]["url"]
    assert "Resolve-Payload" in BUILD
